"""
Run the classifier, using the weights created during the
formatterTraining.py run
"""
import os
import shutil
...
for i in os.listdir("../../data/3_formatting/1_unlabeled/"):
    pairing = []
    file_name = f"../../data/3_formatting/1_unlabeled/{i}"
    print(file_name)
    test_data = pd.read_json(file_name)
    input_x = []
    for section in test_data["sections"]:
        input_x.append(section['content'])
    predictions = classifier_model.predict(input_x)
    for j in range(len(predictions)):
        pairing.append({
            "content": input_x[j], 
            "classification": encoder.get_feature_names()[np.argmax(predictions[j])].split("_")[1]
        })
    print(i)
    with open(f'../../data/3_formatting/3_output/{i}', 'w') as f:
